...
public class Simulator {
	...
	public void simulate() {
		...
		Spot waitingToPark=null;

		while (clock < steps) {
	
			// WRITE YOUR CODE HERE!
			//car arrival this second
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)) {
				Car newCar= RandomGenerator.generateRandomCar();
				Spot arriving = new Spot(newCar, clock);
				incomingQueue.enqueue(arriving);
			}
			int i;
			int j;
			for(i = 0; i<lot.getNumRows(); i++) {
				for (j=0; j<lot.getNumSpotsPerRow(); j++) {
					Spot parked = lot.getSpotAt(i,j);
					
					if (parked!=null) {
						int duration = clock - parked.getTimestamp();
						
						//max duration reached->must leave
						if (duration>= MAX_PARKING_DURATION) {
							Spot removed = lot.remove(i,j);
							if (removed !=null) {
								outgoingQueue.enqueue(removed);
							}
						} else {
							Rational leaveProbability = departurePDF.pdf(duration);
							if (RandomGenerator.eventOccurred(leaveProbability)) {
								Spot removed = lot.remove(i, j);
								if (removed!=null) {
									outgoingQueue.enqueue(removed);
								}
			...
			if (!outgoingQueue.isEmpty()){
				...
				Car c= leaving.getCar();
				System.out.println(c+" EXITED at timestep "+clock+"; occupancy is at " +lot.getTotalOccupancy());
			}
			if (waitingToPark ==null){
				if(!incomingQueue.isEmpty()) {
					waitingToPark = incomingQueue.dequeue();
				}
			}
			if (waitingToPark != null) {
				Car c = waitingToPark.getCar();
				
				boolean success = lot.attemptParking(c, clock);
